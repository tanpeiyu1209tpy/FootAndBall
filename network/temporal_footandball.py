...
def model_factory(model_name, phase, max_player_detections=100, max_ball_detections=100, 
                  player_threshold=0.0, ball_threshold=0.0, **kwargs):
    """
    UPDATED: Support both single-frame and temporal models
    """
    if model_name == 'fb1':
    ...
    elif model_name == 'temporal_fb1':
        # Import temporal model builder
        try:
            from network.temporal_footandball import build_temporal_footandball_detector
        except ImportError:
            # If temporal modules are in the main file, use relative import
            from .temporal_footandball import build_temporal_footandball_detector
            
        temporal_frames = kwargs.get('temporal_frames', 3)
        use_motion_module = kwargs.get('use_motion_module', True)
        
        return build_temporal_footandball_detector(
            phase, 
